...
func TestStsCanaryUpdate(t *testing.T) {
	...
	// Canary update
	g.Expect(c.Get(context.TODO(), client.ObjectKey{Namespace: instance.Namespace, Name: instance.Name}, instance)).Should(gomega.BeNil())
	cu := NewCanaryUpdate(&appsv1alpha1.ManualUpdate{
		Partitions: map[string]int32{
			"subset-a": 6,
			"subset-b": 6,
		}}, map[string]int32{
	...
	for cu.mu != nil {
		t.Logf("Partitions:%v\n", cu.mu.Partitions)
		// Update
		instance.Spec.UpdateStrategy.ManualUpdate = cu.mu
		g.Expect(c.Update(context.TODO(), instance)).Should(gomega.BeNil())

		validate(g, requests, stsList, instance, cu.mu)
...
func validate(g *gomega.GomegaWithT, requests chan reconcile.Request, stsList *appsv1.StatefulSetList, instance *appsv1alpha1.UnitedDeployment, nmu *appsv1alpha1.ManualUpdate) {
	waitReconcilerProcessFinished(g, requests, 6)
	stsList = expectedStsCount(g, instance, 2)
	g.Expect(stsList.Items[0].Spec.Template.Spec.Containers[0].Image).Should(gomega.BeEquivalentTo("nginx:2.0"))
	g.Expect(stsList.Items[1].Spec.Template.Spec.Containers[0].Image).Should(gomega.BeEquivalentTo("nginx:2.0"))
	...
	stsA := getSubsetByName(stsList, "subset-a")
	g.Expect(stsA).ShouldNot(gomega.BeNil())
	g.Expect(*stsA.Spec.UpdateStrategy.RollingUpdate.Partition).Should(gomega.BeEquivalentTo(nmu.Partitions["subset-a"]))

	stsB := getSubsetByName(stsList, "subset-b")
	g.Expect(stsB).ShouldNot(gomega.BeNil())
	g.Expect(*stsB.Spec.UpdateStrategy.RollingUpdate.Partition).Should(gomega.BeEquivalentTo(nmu.Partitions["subset-b"]))

	g.Expect(c.Get(context.TODO(), client.ObjectKey{Namespace: instance.Namespace, Name: instance.Name}, instance)).Should(gomega.BeNil())
	g.Expect(instance.Status.UpdateStatus.CurrentPartitions).Should(gomega.BeEquivalentTo(map[string]int32{
		"subset-a": nmu.Partitions["subset-a"],
		"subset-b": nmu.Partitions["subset-b"],
	}))
}
